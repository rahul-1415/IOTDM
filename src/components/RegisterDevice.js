...
import { keccak256, addHexPrefix, setLengthLeft } from 'ethereumjs-util';
import { merkleRoot } from 'merkle-tree-solidity';

import React, { Component } from 'react';
import './RegisterDevice.css';

import { Steps, Button, Input, Card, Spin, Alert, Divider, Form, Dropdown, Menu, message, notification } from 'antd';
...
class RegisterDevice extends Component {
  ...
  async watchForChanges(txHash) {
    ...
    deviceCreatedEvent.on('data', handleData);
  
    deviceCreatedEvent.on('error', (error) => {
      console.error(error);
    });
  ...
  next = () => {
    const { current, identifier } = this.state;
  ...
  calculateMetadataHash() {
      let elements = this.state.metadata.map(el => keccak256(Buffer.from(el.value)));
      console.log(`Generating Merkle root hash`);
  
      let metadataRootSha3 = merkleRoot(elements).toString('hex');
      console.log(`Merkle root hash: ${metadataRootSha3}`);
  
      this.setState({
        metadataHash: metadataRootSha3
      });
    }

  calculateFirmwareHash() {
    let firmwareBuffer = Buffer.from(this.state.firmware, 'utf8');
    let firmwareHash = keccak256(firmwareBuffer);
  ...
  async createDevice() {
    ...
    try {
      ...
      let result = await instance.createDevice(addHexPrefix(identifierToSave), addHexPrefix(metadataHash), addHexPrefix(firmwareHash), { from: defaultAccount });
