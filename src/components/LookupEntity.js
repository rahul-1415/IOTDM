...
class LookupEntity extends Component {
  ...
  async lookupEntity(address) {
    ...
    try {
      ...
      let filteredData = logs.filter(
        (el) =>
          eventsToSave.includes(el.event) &&
          (el.args.owner === address ||
            el.args.oldOwner === address ||
            el.args.newOwner === address ||
            el.args.signer === address)
      );
  ...
  render() {
    return (
      <div>
        ...
        />
        <br /><br />
        <Button size="small" onClick={() => this.setState({ searchValue: "0x1246a6C5Be0cFf27248556B547FA3Ba1144157cE" })}>Set to my address</Button>
        {this.state.searched &&
          <div>
            <Divider />
