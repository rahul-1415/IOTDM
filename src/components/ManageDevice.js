...
import { Tag, Button, Input, Card, Timeline, Divider, Spin, Alert, notification, message } from 'antd';
...
class ManageDevice extends Component {
  ...
  async updateDeviceData() {
    try {
      ...
      let pastEvents = await instance.getPastEvents('allEvents', {
        filter: {
      ...
      let filteredData = pastEvents.filter(el => eventsToSave.includes(el.event));
      this.setState({
        data: filteredData,
        loading: false,
        owner: device[0],
        ...
        identifierNew: device[1],
        metadataHashNew: device[2],
        firmwareHashNew: device[3],
        ownerNew: device[0]
      })
    } catch (error) {
      console.log(error);
      ...
      switch (property) {
        case 'identifier':
          if (identifier !== identifierNew) {
            ...
            await instance.updateIdentifier(deviceId, addHexPrefix(identifierNew), { from: defaultAccount });
        ...
        case 'metadata':
          if (metadataHash !== metadataHashNew) {
            let defaultAccount = await getDefaultAccount();
            await instance.updateMetadataHash(deviceId, addHexPrefix(metadataHashNew), { from: defaultAccount });
        ...
        case 'firmware':
          if (firmwareHash !== firmwareHashNew) {
            let defaultAccount = await getDefaultAccount();
            await instance.updateFirmwareHash(deviceId, addHexPrefix(firmwareHashNew), { from: defaultAccount });
            this.watchForChanges(property + ' hash');
        ...
        case 'transfer':
          if (owner !== ownerNew) {
            let defaultAccount = await getDefaultAccount();
            await instance.transferDevice(deviceId, addHexPrefix(ownerNew), { from: defaultAccount });
    ...
    let identifierContent = () => {
      ...
      return (
        <div>
          ...
          {
          owner === "0x1246a6C5Be0cFf27248556B547FA3Ba1144157cE" &&
